...
For this project I will be implementing a number of functions which
is used to fit a non-linear function to 1-dimensional data using
Gaussian process regression. 
...
of observations of `x` and `y` pairs.
...
which implies that the observed values of
...
structure. For the sake of simplicity, I assume that
...
these three parameters given the data. I will be accomplishing this via
maximum likelihood. Given the multivariate normal model I can then take
the MVN density,
...
I can derive the log likelihood as
...
Now I will implement a python function which
...
-   My function will return a list with the MLEs of
    ![\\sigma^2_n](https://latex.codecogs.com/svg.latex?%5Csigma%5E2_n "\sigma^2_n"),
    ![\\sigma^2_s](https://latex.codecogs.com/svg.latex?%5Csigma%5E2_s "\sigma^2_s"),
...
-   I will make sure that all example calls to `fit()` do not use the same
    seed.
...
(i.e. draw samples from) the Gaussian process model for new values of
![x](https://latex.codecogs.com/svg.latex?x "x"). Specifically, I want
to provide an fine, equally space grid of
![x](https://latex.codecogs.com/svg.latex?x "x") values from which I
...
-   I will make sure that all example calls to `predict()` do not use the same
    seed.
...
For this step I will take the result from the `predict()`
...
-   I will write a function named `plot_gp()` which takes the following
    arguments:
